...
class QueryTests: XCTestCase {
    ...
    let admin = SQLiteVec.Expression<Bool>("admin")
    let optionalAdmin = SQLiteVec.Expression<Bool?>("admin")

    let posts = Table("posts")
    let userId = SQLiteVec.Expression<Int64>("user_id")
    let categoryId = SQLiteVec.Expression<Int64>("category_id")
    let published = SQLiteVec.Expression<Bool>("published")

    let categories = Table("categories")
    let tag = SQLiteVec.Expression<String>("tag")
