...
class QueryIntegrationTests: SQLiteTestCase {
    ...
    let age = SQLiteVec.Expression<Int>("age")
    ...
    func test_select() throws {
        let managerId = SQLiteVec.Expression<Int64>("manager_id")
    ...
    func test_prepareRowIterator() throws {
        ...
        let emailColumn = SQLiteVec.Expression<String>("email")
    ...
    func test_select_codable() throws {
        ...
        try db.run(table.create { builder in
            builder.column(SQLiteVec.Expression<Int>("int"))
            builder.column(SQLiteVec.Expression<String>("string"))
            builder.column(SQLiteVec.Expression<Bool>("bool"))
            builder.column(SQLiteVec.Expression<Double>("float"))
            builder.column(SQLiteVec.Expression<Double>("double"))
            builder.column(SQLiteVec.Expression<Date>("date"))
            builder.column(SQLiteVec.Expression<UUID>("uuid"))
            builder.column(SQLiteVec.Expression<String?>("optional"))
            builder.column(SQLiteVec.Expression<Data>("sub"))
        })
    ...
    func test_insert_custom_encodable_type() throws {
        ...
        try db.run(table.create { builder in
            builder.column(SQLiteVec.Expression<Int?>("myInt"))
            builder.column(SQLiteVec.Expression<String?>("myString"))
            builder.column(SQLiteVec.Expression<String?>("myOptionalArray"))
        })
    ...
    func test_union() throws {
        ...
        let query3 = users.select(users[*], SQLiteVec.Expression<Int>(literal: "1 AS weight")).filter(email == "sally@example.com")
        let query4 = users.select(users[*], SQLiteVec.Expression<Int>(literal: "2 AS weight")).filter(email == "alice@example.com")

        let sql = query3.union(query4).order(SQLiteVec.Expression<Int>(literal: "weight")).asSQL()
        XCTAssertEqual(sql,
        """
        ...
        let orderedIDs = try db.prepare(query3.union(query4).order(SQLiteVec.Expression<Int>(literal: "weight"), email)).map { $0[id] }
        XCTAssertEqual(Array(expectedIDs.reversed()), orderedIDs)
    }

    func test_no_such_column() throws {
        let doesNotExist = SQLiteVec.Expression<String>("doesNotExist")
    ...
    // https://github.com/stephencelis/SQLiteVec.swift/issues/285
    func test_order_by_random() throws {
        try insertUsers(["a", "b", "c'"])
        let result = Array(try db.prepare(users.select(email).order(SQLiteVec.Expression<Int>.random()).limit(1)))
    ...
    func test_with_recursive() throws {
        ...
        let id = SQLiteVec.Expression<Int64>("id")
        let parent = SQLiteVec.Expression<Int64?>("parent")
        let value = SQLiteVec.Expression<Int64>("value")
