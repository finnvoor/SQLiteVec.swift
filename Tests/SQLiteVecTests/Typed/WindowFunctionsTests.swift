...
class WindowFunctionsTests: XCTestCase {
    ...
    func test_lag_wrapsExpressionWithOverClause() {
        ...
        assertSQL("lag(\"int\", 1, 3) OVER (ORDER BY \"int\" DESC)", int.lag(offset: 1, default: SQLiteVec.Expression<Int>(value: 3), int.desc))
    }
    ...
    func test_lead_wrapsExpressionWithOverClause() {
        ...
        assertSQL("lead(\"int\", 1, 3) OVER (ORDER BY \"int\" DESC)", int.lead(offset: 1, default: SQLiteVec.Expression<Int>(value: 3), int.desc))
    }
