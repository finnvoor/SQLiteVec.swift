...
class ConnectionTests: SQLiteTestCase {
    ...
    func test_init_withURI_returnsURIConnection() throws {
        let db = try Connection(.uri("\(NSTemporaryDirectory())/SQLiteVec.swift Tests.sqlite3"))
        let url = URL(fileURLWithPath: db.description)
        XCTAssertEqual(url.lastPathComponent, "SQLiteVec.swift Tests.sqlite3")
    }

    func test_init_withString_returnsURIConnection() throws {
        let db = try Connection("\(NSTemporaryDirectory())/SQLiteVec.swift Tests.sqlite3")
        let url = URL(fileURLWithPath: db.description)
        XCTAssertEqual(url.lastPathComponent, "SQLiteVec.swift Tests.sqlite3")
    }
    ...
    func test_lastInsertRowid_doesNotResetAfterError() throws {
        ...
        XCTAssertThrowsError(
            ...
            if case SQLiteVec.Result.error(_, let code, _) = error {
    ...
    // https://github.com/stephencelis/SQLiteVec.swift/issues/1071
    #if !os(Linux)
