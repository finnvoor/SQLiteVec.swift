...
class CoreFunctionsTests: XCTestCase {
    ...
    func test_random_generatesExpressionWithRandomFunction() {
        assertSQL("random()", SQLiteVec.Expression<Int64>.random())
        assertSQL("random()", SQLiteVec.Expression<Int>.random())
    }
    ...
    func test_like_buildsExpressionWithLikeOperator() {
        ...
        assertSQL("(\"string\" LIKE \"a\")", string.like(SQLiteVec.Expression<String>("a")))
        assertSQL("(\"stringOptional\" LIKE \"a\")", stringOptional.like(SQLiteVec.Expression<String>("a")))

        assertSQL("(\"string\" LIKE \"a\" ESCAPE '\\')", string.like(SQLiteVec.Expression<String>("a"), escape: "\\"))
        assertSQL("(\"stringOptional\" LIKE \"a\" ESCAPE '\\')", stringOptional.like(SQLiteVec.Expression<String>("a"), escape: "\\"))
