...
class SchemaReaderTests: SQLiteTestCase {
    ...
    func test_foreignKeys() throws {
        ...
        let idColumn = SQLiteVec.Expression<Int64>("id")
        let testIdColumn = SQLiteVec.Expression<Int64>("test_id")

        try db.run(linkTable.create(block: { definition in
            definition.column(idColumn, primaryKey: .autoincrement)
            definition.column(testIdColumn, unique: false, check: nil, references: users, SQLiteVec.Expression<Int64>("id"))
        }))
    ...
    func test_objectDefinitions_indexes() throws {
        let emailIndex = users.createIndex(SQLiteVec.Expression<String>("email"), unique: false, ifNotExists: true)
