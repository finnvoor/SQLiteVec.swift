...
class FTS4Tests: XCTestCase {
    ...
    func test_match_onVirtualTableAsExpression_compilesMatchExpression() {
        assertSQL("(\"virtual_table\" MATCH 'string')", virtualTable.match("string") as SQLiteVec.Expression<Bool>)
        assertSQL("(\"virtual_table\" MATCH \"string\")", virtualTable.match(string) as SQLiteVec.Expression<Bool>)
        assertSQL("(\"virtual_table\" MATCH \"stringOptional\")", virtualTable.match(stringOptional) as SQLiteVec.Expression<Bool?>)
    }
